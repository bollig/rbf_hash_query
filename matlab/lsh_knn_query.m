...
function [stencils] = lsh_knn_query(sorted_nodes, cell_hash, cell_props, node_list, nb_nodes, hnx, max_st_size)
...
for  p = 1:nb_nodes
    ...
    % though?)
    xc = floor((node(1) - cell_props.xmin) / cell_props.cdx);
    ...
    %
    if (xc == cell_props.hnx)
        xc = xc-1;
    end
    
    if (cell_props.cdy > 0)
        yc = floor((node(2) - cell_props.ymin) / cell_props.cdy);
        if (yc == cell_props.hny)
            yc = yc-1;
        end
    else
        yc = 0;
    ...
    if (cell_props.cdz > 0)
        zc = floor((node(3) - cell_props.zmin) / cell_props.cdz);
        if (zc == cell_props.hnz)
            zc = zc-1;
        end
    else
        zc = 0;
